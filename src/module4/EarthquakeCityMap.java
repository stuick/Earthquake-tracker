...
//		fill(color(0, 0, 255));
		rectMode(CENTER);
		rect(50, 180, 20, 20);
		int deep = color(255,0,0);		
		int shallow = color(0, 0, 255);
		int intermediate = color(225, 255, 0);
		fill(deep);
		ellipse(50, 240, 15, 15);
		fill(intermediate);
		ellipse(50, 265, 15, 15);
		fill(shallow);
		ellipse(50, 290, 15, 15);
		pushStyle();
		fill(255,255,255);
		ellipse(50, 315, 15, 15);
		fill(0,0,0);
		line(50-12,315-12,50+12,315+12);
		line(50-12,315+12,50+12,315-12);	
		popStyle();
		
		fill(0, 0, 0);
		text("City Markers", 70,120);
		text("Land quake", 70, 150);
		text("Ocean quake", 70, 180);
		text("Size ~ Magnitude", 40, 210);
		text("Deep", 70, 240);
		text("intermediate", 70, 265);
		text("shalllow", 70, 290);
		text("Past Day",70,315);
	}
		...
		for(Marker m : quakeMarkers){
			if(m instanceof LandQuakeMarker ) {
				...
				if(countryData.containsKey(country)) {
					countryData.put(country, countryData.get(country)+1);
				}else {
					countryData.put(country,1);
				}
			}else if (m instanceof OceanQuakeMarker){
				oceanQuake++;
			}
		}
		System.out.println("List of quake");
		for(String s : countryData.keySet()) {
			System.out.println(s+": "+countryData.get(s));
		}
