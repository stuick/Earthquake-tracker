...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked()
	{
		...
		selectInRadius(quakeMarkers,cityMarkers);
		//hideMarkers();
	...
	private void selectInRadius(List<Marker> earthquake, List<Marker> city) {
		if(lastClicked instanceof EarthquakeMarker) {
			...
			System.out.println(threatCircle);
			for(Marker marker : city) {
				Location loc = marker.getLocation();
				double distance = lastClicked.getDistanceTo(loc);
				if(distance<=threatCircle) {
					((CityMarker)marker).setClicked(true);
					System.out.println(distance);
					((CityMarker)marker).setHidden(false);
					continue;
				}
			...
			for(Marker marker : earthquake) {
				double threatCircle =((EarthquakeMarker)marker).threatCircle();
				double distance = marker.getDistanceTo(loc);
				if(distance<=threatCircle) {
					((EarthquakeMarker)marker).setClicked(true);
					((EarthquakeMarker)marker).setHidden(false);
					continue;
				}
				marker.setHidden(true);
			}
...
//				CommonMarker marker = (CommonMarker)mark;				
				mark.setHidden(true);	
			}
		}
